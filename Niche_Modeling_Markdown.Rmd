...
# select only the variables that are needed
  unique() %>%
# get only unique combinations so that we do not have dublicate points
  dplyr::filter(Longitude > -86.5 & Longitude < -79.5 & Latitude > 24)
# constrain the points to Florida because that is the only data used in the analysis
Map.Florida<- ggplot(data = world) +
# use the background world map
    geom_sf(fill= "antiquewhite", 
            color = "black")+
# controls the color of the land.
  geom_point(data = occs, 
             ...
             color = "white",
             fill = "black") +
# plots the points and controls the size/shape
  coord_sf(xlim = c(-85, 
                    -79.5),
           ylim = c(24,
                    30)) +
# zooms in to only include the state of Florida
  annotation_scale(location = "br",
                   width_hint = 0.5) +
# places a scale bar in the bottom right
  annotation_north_arrow(location = "tr", 
                         which_north = "true", 
        style = north_arrow_fancy_orienteering) +
# places a north arrow in the top right
  xlab("Longitude") + 
  ylab("Latitude") + 
...
# controls the text font, the guidelines, the ocean color, the margins, and the title location
